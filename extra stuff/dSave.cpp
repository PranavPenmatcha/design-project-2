...
// Walking band 0.5–3.5 Hz applied to both pipelines.
constexpr uint16_t FFT_SIZE         = 128;
constexpr uint16_t FFT_STRIDE       = 26;
constexpr float    WALK_BAND_LO_HZ  = 0.5f;
constexpr float    WALK_BAND_HI_HZ  = 8.0f;  // raised: 3.5 was capping fast motion
constexpr float    BAND_ENERGY_FLOOR = 0.0005f;

// Pipeline 1: accel magnitude
static float    sampleBufMag[FFT_SIZE];
static uint16_t sampleHeadMag  = 0;
static uint16_t sampleFillMag  = 0;
static uint16_t strideCountMag = 0;
float peakFrequencyHzMag = 0.0f;
float bandEnergyMag      = 0.0f;

// Pipeline 2: raw Z-axis accel (individual-axis approach)
static float    sampleBufZ[FFT_SIZE];
static uint16_t sampleHeadZ  = 0;
static uint16_t sampleFillZ  = 0;
static uint16_t strideCountZ = 0;
...
constexpr float TREMOR_ENERGY_MIN      = 0.005f;  // floor for any classification
constexpr float DYSKINESIA_ENERGY_MIN  = 0.025f;  // dyskinesia is high-amplitude
constexpr float BRADYKINESIA_ENERGY_MAX = 0.015f; // bradykinesia is low-amplitude
constexpr float BRADYKINESIA_ENERGY_MIN = 0.0008f;
...
// Shared FFT energy helper: copies circular buf oldest-first into fftReal/fftImag,
// removes DC, applies Hann, runs FFT, then extracts peak Hz and band energy.
//
// Band energy via Parseval's theorem for a real signal:
//   E_band = (2/N) * sum_{k=b0}^{b1} |X[k]|^2
// The factor of 2 folds in the negative-frequency mirror bins, giving the
// physically correct one-sided power spectral estimate. Without it the energy
// is half-scale and the BAND_ENERGY_FLOOR suppression fires too late, leaving
// the peak stuck at the top boundary bin (3.66 Hz = bin 9).
static void computeFftEnergyFrom(float* buf, uint16_t head, uint16_t fill,
                                  float& outHz, float& outEnergy) {
  const uint16_t oldest = (fill == FFT_SIZE) ? head : 0u;
  for (uint16_t i = 0; i < FFT_SIZE; i++) {
    fftReal[i] = buf[(oldest + i) % FFT_SIZE];
    fftImag[i] = 0.0f;
  }
  ...
  // Band bin limits. Use floor() for kLo and ceil()-1 for kHi so the boundary
  // bins (which straddle the band edge) are excluded — this prevents the peak
  // from anchoring to the exact boundary frequency (was always landing at 3.66).
  ...
  const uint16_t kHi = static_cast<uint16_t>(
      WALK_BAND_HI_HZ * static_cast<float>(FFT_SIZE) / SAMPLE_RATE_HZ);  // floor, not round
  ...
  // Parseval's theorem for one-sided real-signal spectrum: E = 2/N * sum|X[k]|^2
  outEnergy = 2.0f * bandPwr / static_cast<float>(FFT_SIZE);
...
static void drawDisplay(uint16_t steps, float cadence,
  ...
  // Line 3: Hz from magnitude and Z FFT pipelines
  snprintf(line, sizeof(line), "M:%.2f Z:%.2f", hzMag, hzZ);
  u8x8.setCursor(0, 3); u8x8.print(line);

  // Line 4: band energy (magnitude pipeline)
  snprintf(line, sizeof(line), "Eb:%.5f", engBand);
...
void loop() {
  ...
  // Pipeline 1: magnitude FFT
  sampleBufMag[sampleHeadMag] = mag;
  sampleHeadMag = (sampleHeadMag + 1) % FFT_SIZE;
  if (sampleFillMag < FFT_SIZE) sampleFillMag++;
  strideCountMag++;
  if (sampleFillMag >= FFT_SIZE && strideCountMag >= FFT_STRIDE) {
    strideCountMag = 0;
    computeFftEnergyFrom(sampleBufMag, sampleHeadMag, sampleFillMag,
                         peakFrequencyHzMag, bandEnergyMag);
  }

  // Pipeline 2: Z-axis FFT (raw individual axis, per test-code read pattern)
  sampleBufZ[sampleHeadZ] = az;
  sampleHeadZ = (sampleHeadZ + 1) % FFT_SIZE;
  if (sampleFillZ < FFT_SIZE) sampleFillZ++;
  strideCountZ++;
  if (sampleFillZ >= FFT_SIZE && strideCountZ >= FFT_STRIDE) {
    strideCountZ = 0;
    computeFftEnergyFrom(sampleBufZ, sampleHeadZ, sampleFillZ,
                         peakFrequencyHzZ, bandEnergyZ);
  }
  ...
  if (nowMs >= nextDisplayMs) {
    ...
    drawDisplay(steps, cadenceSpm, peakFrequencyHzMag, peakFrequencyHzZ,
                bandEnergyMag, g_motionState, g_tremorType);
  }
  ...
  if (nowMs >= nextBleMs) {
    ...
    g_tremorType  = classifyTremor(peakFrequencyHzMag, peakFrequencyHzZ,
